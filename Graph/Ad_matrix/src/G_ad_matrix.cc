...
Graph::Graph(int numbers_v){
    ...
    for(int i = 0; i < n_vertexs; i++){
        ...
        for(int j = 0; j < n_vertexs; j++){
            adjacency_matrix[i][j] = noEdge; // noEdge init "-1"
        }
...
void Graph::add_edge(int vertex1, int vertex2, int weight){
    ...
    if(adjacency_matrix[vertex1][vertex2] != noEdge || adjacency_matrix[vertex2][vertex1] != noEdge){
...
void Graph::del_edge(int vertex1, int vertex2){
    ...
    adjacency_matrix[vertex1][vertex2] = noEdge;
    adjacency_matrix[vertex2][vertex1] = noEdge;
    n_edges -= 1;
}
